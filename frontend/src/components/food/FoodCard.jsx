...
function getUrgency(expiryTime) {
  const minutes = Math.max(0, Math.round((new Date(expiryTime) - Date.now()) / 60000));
...
export function FoodCard({ food, compact = false }) {
  ...
  const discount = Math.round((1 - pricing.discounted / pricing.original) * 100);
  ...
  if (isExpired || isOutOfStock) {
    ...
    return (
      <div className="bg-white rounded-2xl overflow-hidden shadow-sm border border-gray-100 opacity-50 cursor-not-allowed">
        <div className={`relative overflow-hidden ${compact ? 'h-36' : 'h-44'} bg-gray-100 flex items-center justify-center`}>
        ...
        <div className="p-3">
          ...
          <h3 className={`font-bold text-gray-900 ${compact ? 'text-sm' : 'text-base'}`}>{food.name}</h3>
          <div className="flex items-center gap-2 mt-1">
            <span className="text-xl font-black text-gray-400">₹{pricing.discounted}</span>
            <span className="text-sm text-gray-300 line-through">₹{pricing.original}</span>
          </div>
        </div>
      </div>
    );
  ...
  return (
    <Link href={`/discover/${food._id}`} className="group block">
      <div className="bg-white rounded-2xl overflow-hidden shadow-sm card-hover border border-gray-100">
        ...
        <div className={`relative overflow-hidden ${compact ? 'h-36' : 'h-44'} bg-gray-100`}>
          {food.image ? (
            <img
              src={food.image}
              alt={food.name}
        ...
        <div className="p-3">
          <div className="flex items-center justify-between mb-0.5">
            <span className="text-xs text-gray-500 font-medium truncate max-w-[70%]">
              {restaurant?.name}
            </span>
          ...
          <h3 className={`font-bold text-gray-900 leading-tight mb-2 ${compact ? 'text-sm' : 'text-base'}`}>
            {food.name}
          </h3>
          ...
          {!compact && tags.length > 0 && (
            <div className="flex flex-wrap gap-1 mb-2.5">
              {tags.slice(0, 3).map(tag => (
                <span
                  key={tag}
                  className="px-2 py-0.5 rounded-full text-xs bg-orange-50 text-orange-700 capitalize"
                >
                  {tag}
                </span>
              ))}
          ...
          <div className="flex items-center gap-2">
            ...
            <span className="text-sm text-gray-400 line-through">₹{pricing.original}</span>
